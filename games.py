...
def query_player(game, state):
    ...
    while move not in game.actions(state):
        ...
        try:
            ...
            move = move_string
        if move not in game.actions(state):  # Check if the move is in the available moves
...
def alpha_beta_player(game, state):
    ...
    game.display(state)
    print("available moves: {}".format(game.actions(state)))
    x = alpha_beta_search(state, game)
    print("Taken action: ", x)
    return x
